...
if __name__ == "__main__":
    ...
    if len(args.alphas) == 1:
        n = int(round(args.alphas[0]))
        alpha = torch.arange(1, n - 1, device=args.device) / n * torch.pi / 2
        beta1 = torch.tensor((n - 1) / n * torch.pi / 2, device=args.device)
        beta2 = torch.tensor(torch.pi / 2, device=args.device)
    else:
        alpha = torch.tensor(args.alphas, device=args.device)
        beta1 = torch.tensor(args.beta1, device=args.device)
    ...
    for epoch in progress_bar:
        ...
        area = compute_area(alpha, beta1, beta2, u1, u2,
