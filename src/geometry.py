...
def interp1d_sorted(x0, y0, x1, outside_value=0.):
    assert len(x0) >= 2
...
def interp1d_multi_section(x, y, x_tar, outside_value, min_split_reduce):
    # split curve
    ...
    for j, (x_split, y_split) in enumerate(zip(x_splits, y_splits)):
        out[j] = interp1d_sorted(x_split, y_split, x_tar, outside_value=outside_value)
    # reduce splits
...
def compute_area(alpha, xp, yp, xp_prime, yp_prime, n_area_samples=2000, return_outline=False):
    ...
    # lower edge
    y_sample_p = interp1d_multi_section(xp, yp, x_sample, outside_value=0., min_split_reduce=False)
    y_sample_u = interp1d_multi_section(xu, yu, x_sample, outside_value=0., min_split_reduce=False)
